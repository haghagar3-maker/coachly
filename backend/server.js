...
app.post('/api/program/generate', requireAuth, requireUser, async (req, res) => {
  try {
    ...
    const coach = coaches[0];
    const user = { ...users[0], ...(subs?.[0]?.intake || {}) };
    const prompt = `Create a detailed 5-day workout program. Respond ONLY with a JSON array, no text before or after. Each day must have 4-6 exercises. Example format:\n[{"day_name":"Monday","session_title":"Upper Body Strength","exercises":[{"name":"Push-ups","sets":4,"reps":"15","rest":"45s"},{"name":"Dumbbell Rows","sets":3,"reps":"12","rest":"60s"},{"name":"Shoulder Press","sets":3,"reps":"10","rest":"60s"},{"name":"Tricep Dips","sets":3,"reps":"12","rest":"45s"},{"name":"Plank","sets":3,"reps":"45 seconds","rest":"30s"}]}]\nNow create a 5-day program for a ${coach.sport || 'fitness'} coach's client whose goal is ${user.goal || 'lose weight'}. Coach method: ${coach.ai_method || 'progressive training'}. Each day MUST have 4-6 exercises with sets, reps and rest. Return ONLY the JSON array.`;
