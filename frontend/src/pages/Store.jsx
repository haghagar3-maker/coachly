      ...
      <div className="store-main-grid" style={{ maxWidth: '900px', margin: '0 auto', padding: '40px 24px', display: 'grid', gridTemplateColumns: '1fr 340px', gap: '32px', alignItems: 'start' }}>
        ...
        <div>
          ...
          {activeTab === 'community' && (
            <div>
              {!currentUser ? (
                <div style={{ borderRadius: '16px', minHeight: '380px', background: '#fff', border: '1px solid #eee', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '16px', padding: '40px', textAlign: 'center' }}>
                  <div style={{ fontSize: '48px' }}>🔒</div>
                  <div style={{ fontSize: '18px', fontWeight: '800', color: '#111', fontFamily: 'Georgia, serif', maxWidth: '320px', lineHeight: '1.4' }}>"The people around you shape who you become."</div>
                  <div style={{ fontSize: '13px', color: '#888', maxWidth: '300px', lineHeight: '1.6' }}>Join the community — share your wins, get support, and stay accountable.</div>
                  <button onClick={handleSubscribeClick} style={{ padding: '14px 32px', borderRadius: '100px', border: 'none', background: accentColor, color: isDarkAccent ? '#111' : '#fff', fontSize: '14px', fontWeight: '800', cursor: 'pointer', fontFamily: 'inherit', boxShadow: `0 4px 20px ${accentColor}66` }}>Subscribe to join →</button>
                </div>
              ) : posts.length === 0 ? (
